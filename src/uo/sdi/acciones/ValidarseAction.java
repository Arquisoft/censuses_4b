...
public class ValidarseAction implements Accion {
	...
	if (session.getAttribute("user") == null) {
		...
		userByLogin = dao.findByLoginPass(nombreUsuario, contraseña);
		if (userByLogin != null) {
		    ...
		    session.invalidate();
		    Log.info(
			    "El usuario [%s] no ha introducido una contraseña correcta",
			    nombreUsuario);
		    resultado = "FRACASO";
		...
		request.setAttribute(
			...
			"El usuario "
				+ nombreUsuario
				+ " no ha podido iniciar sesión porque no está registrado");
		session.invalidate();
